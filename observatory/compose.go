...
func startComposeStack(repoRoot string) error {
	...
	log.Println("pulling/starting Prometheus (first run may take a minute)...")
...
func waitForMonitoringStack() error {
	...
	for time.Now().Before(deadline) {
		...
		if time.Since(lastLog) >= 5*time.Second {
			log.Printf("waiting for Prometheus... ready=%t", promOK)
			lastLog = time.Now()
		}
		time.Sleep(500 * time.Millisecond)
	}
	return fmt.Errorf("Prometheus did not become ready within %s", composeWaitTimeout)
}
